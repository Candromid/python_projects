...
#заполняем список через пробел 
containers = list(map(int, input("Введите вес контейнеров через пробел ").split()))

#Проверка на то , что все веса контейнров не превышают значения 200
if any(x > 200 for x in containers):
    print("Ошибка! Масса контейнера не может превышать значения 200кг! ")
    exit()

#Ввод массы нового контейнера
x = int(input("Введите массу нового контейнера "))
if x > 200:
    print("Ошибка! Масса контейнера не может превышать значения 200кг! ")
    exit()

position = 1 #Позиция контейнеров в списке начинается с 1
for weight in containers:
    if x <= weight: # проверка сравнение веса нового с имеющимися, пока новый легче двигаем указатель вправо
        position += 1

#Вставляем новый контейнер в свое место
containers.insert(position - 1, x)

print(f"Новый контейнер встанет на место №{position}")
print("Обновлённый ряд контейнеров:", containers)
"""
...
# Делаем сдвиг
K %= len(arr)  # если K больше длины списка
arr = arr[-K:] + arr[:-K]

print("Список после сдвига:", arr)
"""
"""
# Задача на пузырьковую сортировку
numbers = list(map(int, input("Заполните список числами через пробел: ").split()))

# пузырьковая сортировка
n = len(numbers)
for i in range(n):  
    for j in range(n - 1 - i):  # пробегаем до предпоследнего
        if numbers[j] > numbers[j + 1]:
            numbers[j], numbers[j + 1] = numbers[j + 1], numbers[j]  # меняем местами
